...
public:
    all_coordinator(promise_ptr<result_type> promise) : 
        promise { promise } 
        {  }

    template<std::size_t ...Indices>
    void attach(std::index_sequence<Indices...>, std::tuple<promise_ptr<T_values> &...> &&promises) {
        ([&, this] (auto &promise, auto &slot) {
            ...
            if constexpr(promise_type::is_void) {
                ...
                promise->then(
                    [&, guard = this->shared_from_this()] (auto &value) { 
...
template<class ...T_values>
auto all(promise_ptr<T_values> ...promises) {
    return make_promise<all_result<T_values...>>([&] (auto &all_promise) {
